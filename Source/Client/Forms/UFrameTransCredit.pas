...
type
  TfFrameTransCredit = class(TfFrameNormal)
...
class function TfFrameTransCredit.FrameID: integer;
begin
  Result := cFI_FrameTransCredit;
end;

procedure TfFrameTransCredit.OnCreateFrame;
begin
  inherited;
  FWhereDtl := '';
  FCreditType := 'MAIN_C13';

  InitDateRange(Name, FStart, FEnd);
end;

procedure TfFrameTransCredit.OnDestroyFrame;
begin
  SaveDateRange(Name, FStart, FEnd);
  inherited;
end;

procedure TfFrameTransCredit.OnLoadGridConfig(const nIni: TIniFile);
begin
...
procedure TfFrameTransCredit.OnSaveGridConfig(const nIni: TIniFile);
begin
  SaveUserDefineTableView(Name, cxView2, nIni);
end;

procedure TfFrameTransCredit.OnInitFormData(var nDefault: Boolean;
...
begin
  ...
  nSQL := MacroValue(nSQL, [MI('$Cus', sTable_Customer),
          MI('$CA', sTable_TransAccount), MI('$SM', sTable_Salesman)]);
  //xxxxx
...
//Desc: 查询明细
procedure TfFrameTransCredit.QueryDetail(const nWhere: string);
var nStr: string;
begin
  ...
  nStr := Format(nStr, [sTable_TransCredit, sTable_Customer]);
...
//------------------------------------------------------------------------------
//Desc: 刷新
procedure TfFrameTransCredit.BtnRefreshClick(Sender: TObject);
begin
  FWhere := '';
  FWhereDtl := '';
  InitFormData;
...
//Desc: 信用变动
procedure TfFrameTransCredit.BtnEditClick(Sender: TObject);
...
begin
  ...
  CreateBaseFormItem(cFI_FormCusCredit, FCreditType, @nP);
...
//Desc: 执行查询
procedure TfFrameTransCredit.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 备注信息
procedure TfFrameTransCredit.cxView2FocusedRecordChanged(
...
//Desc: 筛选明细
procedure TfFrameTransCredit.cxView1DblClick(Sender: TObject);
var nStr: string;
begin
...
//Desc: 筛选日期
procedure TfFrameTransCredit.EditDatePropertiesButtonClick(
...
initialization
  gControlManager.RegCtrl(TfFrameTransCredit, TfFrameTransCredit.FrameID);
end.
