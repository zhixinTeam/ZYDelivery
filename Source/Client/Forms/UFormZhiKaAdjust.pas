{*******************************************************************************
  ...
  备注:
  *.某客户原来有旧的订单,当办理新订单时,需要对旧订单做适当调整.
  *.调整方案分三种:1.将旧订单作废,启用新的订单,原订单的所有磁卡自动过继到新订单
    名下;2.原订单保留,将现有可用金额划转到旧订单中,限制旧订单的可提货量;3.新旧
    订单同时使用.
*******************************************************************************}
...
begin
  ...
  nStr := 'Select * From %s Where Z_Customer=''%s'' and ' +
          ...
          'Order By Z_ID';
  nStr := Format(nStr, [sTable_ZhiKa, nCusID]);
