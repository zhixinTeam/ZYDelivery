...
type
  ...
  TfFormFLZhiKa = class(TfFormNormal)
...
var
  gForm: TfFormFLZhiKa = nil;

//------------------------------------------------------------------------------
class function TfFormFLZhiKa.CreateForm(const nPopedom: string;
...
begin
  ...
  case nP.FCommand of
   cCmd_AddData:
    with TfFormFLZhiKa.Create(Application) do
    begin
      FRecordID := '';
      Caption := '返利订单 - 办理';
   ...
   cCmd_ViewData:
    begin
      ...
      begin
        gForm := TfFormFLZhiKa.Create(Application);
        with gForm do
        begin
...
class function TfFormFLZhiKa.FormID: integer;
begin
  Result := cFI_FormFLZhiKa;
end;

procedure TfFormFLZhiKa.FormCreate(Sender: TObject);
begin
  LoadFormConfig(Self);
...
procedure TfFormFLZhiKa.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  SaveFormConfig(Self);
...
//------------------------------------------------------------------------------
//Desc: 订单编号
procedure TfFormFLZhiKa.InitFormData(const nID: string);
var nStr: string;
    nZK: TZhiKaItem;                     
    i,nIdx: integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      nStr := Format(nStr, [sTable_FLZhiKaDtl, nID]);
...
//Desc: 载入水泥列表
procedure TfFormFLZhiKa.LoadStockList;
...
//Desc: 载入摘要
procedure TfFormFLZhiKa.LoadStockListSummary;
var nMoney: Double;
    nIdx,nNum: integer;
begin
  nNum := 0;
...
procedure TfFormFLZhiKa.LoadSaleContract(const nCID: string);
var nStr: string;
    nIdx: integer;
begin
...
//Desc: 业务员变更,提取相关客户
procedure TfFormFLZhiKa.EditSManPropertiesEditValueChanged(Sender: TObject);
var nStr: string;
begin
...
//Desc: 载入合同
procedure TfFormFLZhiKa.EditCIDExit(Sender: TObject);
begin
...
//Desc: 同步更新办理明细
procedure TfFormFLZhiKa.ListDetailClick(Sender: TObject);
var nIdx: Integer;
    nChanged: Boolean;
begin
  nChanged := False;
...
//Desc: 更新设置
procedure TfFormFLZhiKa.EditPricePropertiesEditValueChanged(Sender: TObject);
var nInt: Integer;
    nChanged: Boolean;
begin
...
//Desc: 快捷菜单
procedure TfFormFLZhiKa.N3Click(Sender: TObject);
var nIdx: integer;
    nBool: Boolean;
begin
  for nIdx:=Low(FStockList) to High(FStockList) do
...
//Desc: 快速选择合同
procedure TfFormFLZhiKa.EditCIDKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
//Desc: 快速选择客户
procedure TfFormFLZhiKa.EditCustomKeyPress(Sender: TObject; var Key: Char);
var nStr: string;
...
procedure TfFormFLZhiKa.EditCIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
...
//Desc: 验证Sender控件
function TfFormFLZhiKa.OnVerifyCtrl(Sender: TObject; var nHint: string): Boolean;
...
begin
  ...
  //限制可用金
  nStr := Format(nStr, [sTable_FLZhiKa, nCusID, sFlag_Yes]);
  nVal := FDM.QueryTemp(nStr).Fields[0].AsFloat;

  nVal := GetCustomerCompensateMoney(nCusID) - nVal;
...
function TfFormFLZhiKa.checkYFMoneyValid(const nCusID: string;
  const nYFMoney: Double): Boolean;
var nVal, f: Double;
    nStr: string;
begin
  Result := False;
  nVal := GetCustomerCompensateMoney(nCusID);
  //可用金额
...
//Desc: 保存数据
procedure TfFormFLZhiKa.BtnOKClick(Sender: TObject);
var nIdx: integer;
...
begin
  ...
  nZID := SaveFLZhiKa(PackerEncodeStr(FListA.Text));
  ...
  if FZhiKa.FCard = '' then SaveICCardInfo(nZID, sFlag_BillFL, sFlag_ICCardM);

  ModalResult := mrOK;
  ShowMsg('订单已保存', sHint);
end;

procedure TfFormFLZhiKa.ListDetailChange(Sender: TObject; Item: TListItem;
  Change: TItemChange);
var nIdx: Integer;
begin
  inherited;
...
procedure TfFormFLZhiKa.EditCustomPropertiesChange(Sender: TObject);
var nCusID: string;
begin
  inherited;
  if FZhiKa.FIsXuNi then
...
initialization
  gControlManager.RegCtrl(TfFormFLZhiKa, TfFormFLZhiKa.FormID);
end.
