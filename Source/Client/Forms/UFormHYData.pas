...
begin
  if nID <> '' then
  begin
    nSQL := 'L_SaleID=Select L_SaleID,L_SaleMan From %s ' +
            'Where L_ID=''%s''';
    nSQL := Format(nSQL, [sTable_Bill, nID]);

    AdjustStringsItem(EditSMan.Properties.Items, True);
    FDM.FillStringsData(EditSMan.Properties.Items, nSQL, -1, '.', DSA(['L_SaleID']));

    AdjustStringsItem(EditSMan.Properties.Items, False);
    if EditSMan.Properties.Items.Count>0 then EditSMan.ItemIndex := 0;

    nSQL := 'L_CusID=Select L_CusID,L_CusName From %s ' +
            'Where L_ID=''%s''';
    nSQL := Format(nSQL, [sTable_Bill, nID]);

    AdjustStringsItem(EditCustom.Properties.Items, True);
    FDM.FillStringsData(EditCustom.Properties.Items, nSQL, -1, '.', DSA(['L_CusID']));

    AdjustStringsItem(EditCustom.Properties.Items, False);
    if EditCustom.Properties.Items.Count>0 then EditCustom.ItemIndex := 0;
    ...
    begin
      ...
      begin
        FLID           := nID;
        EditNo.Text    := FieldByName('L_Seal').AsString;
        EditDate.Date  := FieldByName('L_Date').AsDateTime;
        EditValue.Text := FloatToStr(FieldByName('L_Value').AsFloat);

        EditName.Text  := FieldByName('L_CusName').AsString;
...
begin
  nStr := 'Insert Into $Table(H_Custom,H_CusName,H_SerialNo,H_Truck,H_Value,' +
          'H_BillDate,H_ReportDate,H_Reporter, H_NO) Values(''$Cus'',''$CName'',' +
          '''$No'',''$TN'',$Val,''$BD'',$RD,''$RE'', ''$LID'')';
...
begin
  FRecordID := IntToStr(FDM.GetFieldMax(sTable_StockHuaYan, 'H_ID'));
  //PrintHeGeReport(FRecordID, True);
  PrintHuaYanReport(FRecordID, True);
end;
