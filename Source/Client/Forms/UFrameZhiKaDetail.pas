...
begin
  FCheckField := 'FSelected';
  EditDate.Text := Format('%s 至 %s', [Date2Str(FStart), Date2Str(FEnd)]);

  Result := 'Select ''N'' as $CheckField, sm.*,zk.*,zd.*,ht.*,zd.R_ID as D_RID,' +
...
begin
  ...
  begin
    ...
    if FCheckField<>'' then nSel := GetVal(nIdx, FCheckField);
    if ((nSel = 'Y') or (FCheckField='')) and (nStr <> '')
      and (nList.IndexOf(nStr) < 0) then
      nList.Add(nStr);
    //xxxxx
  end;
end;
